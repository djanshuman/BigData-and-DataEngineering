...
SELECT
  ...
  count(call_duration_secs) as "current_month",
  lag(count(call_duration_secs)) over(order by extract(month from call_date)) as "previous_month"
  from callers
  ...
  SELECT
    yr,
    mth,
    Round(100.0*(current_month - previous_month) / previous_month,1) AS long_calls_growth_pct
    from cte
    order by 1,2;
"""
Output

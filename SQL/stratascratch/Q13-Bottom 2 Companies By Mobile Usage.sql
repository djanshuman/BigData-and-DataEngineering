'''
  https://platform.stratascratch.com/coding/2026-bottom-2-companies-by-mobile-usage?code_type=1
...
select 
    ...
    from (
        ...
        dense_rank() over (order by count(*) asc) rank
        from fact_events
    where client_id = 'mobile'
    group by 1) 
    a where rank =1 order by number_of_events;


'''
  customer_id	number_of_events
  Electric Gravity	4
  eShop	4
'''

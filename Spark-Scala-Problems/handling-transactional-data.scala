...
def etl(df_transactions: DataFrame, df_clients: DataFrame): DataFrame = {
    ...
    val transWindow= Window.partitionBy(col("TransactionID")).orderBy(col("TransactionID"))
    val clientWindow= Window.partitionBy(col("ClientID")).orderBy(col("ClientID"))

    // dedup dataframes and keep first row

    val dedupTransDF= df_transactions
                    .withColumn("rn",row_number().over(transWindow))
                    .filter("rn == 1")
                    .drop(col("rn"))

    val dedupClientDF= df_clients
    ...
    val cleanTransDF= dedupTransDF
                        .filter(col("TransactionID") > 0)
                        .filter(col("Date").rlike("^[0-9]{4}-[0-9]{2}-[0-9]{2}$"))
    val cleanClientDF = dedupClientDF
                        .filter(col("ClientID") > 0)

    val joinedDF= cleanTransDF.join(cleanClientDF,"ClientID")
    joinedDF

...
def etl(social_media: DataFrame): DataFrame = {
    val text = regexp_replace(col("text"),"Python","PySpark")
    val result_df= social_media.withColumn("text",text)
    result_df
    
    /*
    other way
    val result_df= social_media.withColumn
    ("text",regexp_replace($"text","Python","PySpark"))
    */
